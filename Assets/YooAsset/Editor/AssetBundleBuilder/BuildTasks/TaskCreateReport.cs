...
namespace YooAsset.Editor
{
	...
	public class TaskCreateReport : IBuildTask
	{
		...
		private void CreateReportFile(BuildParametersContext buildParametersContext, BuildMapContext buildMapContext, PatchManifestContext patchManifestContext)
		{
            ...
            {
				var packageName = item.Key.Split('_')[1];
				...
				// 概述信息
				{
					...
					buildReport.Summary.BuildPackageName = packageName;
				...
				// 序列化文件
				string fileName = YooAssetSettingsData.GetReportFileName(packageName, buildParameters.PackageVersion);
		...
		{
			List<string> dependBundles = new List<string>();
			foreach (var dependBundleID in patchAsset.DependIDs)
			{
				string dependBundleName = patchManifest.DependBundleNameList[dependBundleID];
